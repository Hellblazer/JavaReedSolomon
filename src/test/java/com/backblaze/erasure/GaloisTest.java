...
/**
 * This is a totally paranoid test that ensure that the Galois class actually
 * implements a field, with all of the properties that a field must have.
 */
public class GaloisTest {
        ...
        for (int i = -128; i < 128; i++) {
            ...
            for (int j = -128; j < 128; j++) {
                ...
                for (int k = -128; k < 128; k++) {
                    byte c = (byte) k;
                    assertEquals(Galois.add(a, Galois.add(b, c)), Galois.add(Galois.add(a, b), c));
                    assertEquals(Galois.multiply(a, Galois.multiply(b, c)), Galois.multiply(Galois.multiply(a, b), c));
                }
            }
        }
    }
    ...
    public void testDistributivity() {
        for (int i = -128; i < 128; i++) {
            ...
            for (int j = -128; j < 128; j++) {
                ...
                for (int k = -128; k < 128; k++) {
                    byte c = (byte) k;
                    assertEquals(Galois.multiply(a, Galois.add(b, c)),
    ...
    public void testGenerateLogTable() {
        ...
        final Integer[] polynomials = { 29, 43, 45, 77, 95, 99, 101, 105, 113, 135, 141, 169, 195, 207, 231, 245 };
